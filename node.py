...
def get_balance():
    ...
    if balance != None:
        response = {
            'message': 'balance successfuly loaded',
            'funds': balance
        }
        return jsonify(response), 200
    else:
...
def broadcast_block():
    ...
    if block['index'] == blockchain.chain[-1].index + 1:
        ...
        response = {
            'message': 'blockchain conflicts with local'
        }
        blockchain.resolve_conflicts = True
        return jsonify(response), 200
    else:
        response = {
            'message': 'blockchain is shorter, block not added'
        }
        return jsonify(response), 409
...
def mine():
    if blockchain.resolve_conflicts:
        response = {
            'message': 'resolve conflicts, block failed'
        }
        return jsonify(response), 409
